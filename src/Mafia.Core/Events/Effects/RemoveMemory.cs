...
public sealed class RemoveMemory(string rootPath, string targetPath, string definitionId) : IEventEffect
{
    public void Apply(EntityScope context)
    {
        ...
        root.Ref<MemoriesOf>(target).Memories.RemoveAll(m => m.DefinitionId == definitionId);
    }
}
