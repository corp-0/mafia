...
public class TargetPoolResolver
{
    ...
    public Entity? ResolveTarget(TargetSelection selection, Entity root, EntityScope scope)
    {
        var pool = Resolve(selection.Pool, root);
        ...
        if (selection.Filter is { } filter)
        {
            ...
            {
                var tempScope = new EntityScope(scope.World)
                {
