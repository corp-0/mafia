...
/// <summary>
/// List of effect entries with add/remove/reorder.
/// Reusable: used in options (standard outcome, success/failure, weighted outcomes).
/// </summary>
public partial class EffectListSection : CollapsibleSection
{
    private VBoxContainer _itemsContainer = null!;
    private readonly List<EffectFormSection> _forms = [];
    private List<EffectDto> _dtos = [];
    ...
    public void LoadFromDtos(List<EffectDto>? dtos)
    {
        _dtos = dtos != null ? [..dtos] : [];
        Rebuild();
    }

    public List<EffectDto>? WriteToDtos()
    {
    ...
    {
        SaveFormState();
        _dtos.Add(new EffectDto { Type = "modify_stat" });
        Rebuild();
    }
    ...
    private void Rebuild()
    {
        ...
        for (var i = 0; i < _dtos.Count; i++)
        {
            ...
            var form = new EffectFormSection();
            form.Initialize(_dtos[i]);
            _forms.Add(form);
